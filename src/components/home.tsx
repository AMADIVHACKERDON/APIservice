...
interface SearchComponentProps {
  categoriesDatabase: Category[];
  personsDatabase: Person[];
}

export default function SearchComponent({ categoriesDatabase, personsDatabase }: SearchComponentProps) {
  const [query, setQuery] = useState('');

  // 1. Find matched category from search query
  const filteredCategories = query && categoriesDatabase.filter((item) =>
    item.name.toLowerCase().includes(query.toLowerCase())
  );
  
  const matchedCategory = filteredCategories && filteredCategories.length > 0 ? filteredCategories[0] : null;

  // 2. Relational Lookup: Find the assigned expert
  const assignedExpert = matchedCategory 
    ? personsDatabase.find(person => person.id === matchedCategory.expertId) || personsDatabase.find(person => person.id === 999)
    : null;

  // 3. Find all related categories
  const relatedCategories = categoriesDatabase.filter(item => matchedCategory?.relatedIds?.includes(item.id));
  ...
  return (
    <div style={{ maxWidth: '650px', margin: '40px auto', padding: '24px', fontFamily: 'system-ui, -apple-system, sans-serif', backgroundColor: '#ffffff', borderRadius: '12px', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }}>
      ...
      {/* NO SEARCH STATE: Display all categories as short cards */}
      {!query && (
        <div>
      ...
      {query && !matchedCategory && (
        <p style={{ textAlign: 'center', color: '#64748b', fontSize: '14px', marginTop: '20px' }}>No results found for "{query}"</p>
      )}
    </div>
  );
}
